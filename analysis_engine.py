"""
SAAI v6.0 - Smart Alert AI System
...
def check_earnings(ticker):
    try:
        ...
        if days_away == 0:
            warn = f"EARNINGS HOY ({date_str}) -- NO ENTRAR. IV Crush garantizado."
            impact = "CRITICO"
        elif days_away == 1:
            warn = f"EARNINGS MANANA ({date_str}) -- Reducir tamano."
            impact = "ALTO"
        else:
            warn = f"EARNINGS en {days_away} dias ({date_str})."
            impact = "MEDIO"
        return {"has_earnings": True, "days_away": days_away, "date": date_str, "warning": warn, "impact": impact}
    except:
        return {"has_earnings": False}
...
def analyze_bb15(df_15m):
    ...
    mid_slope_pct = round((mid_now - mid_3ago) / mid_3ago * 100, 4) if mid_3ago > 0 else 0.0
    mid_alcista = mid_slope_pct > 0.03; mid_bajista = mid_slope_pct < -0.03
    ...
    return BB15Analysis(
        ...
        mid_slope_pct=mid_slope_pct, mid_alcista=mid_alcista, mid_bajista=mid_bajista,
...
def analyze_bb1h(df_1h):
    ...
    return BB1HAnalysis(
        ...
        mid_alcista=slope>0.05, mid_bajista=slope<-0.05,
        espacio_superior_pct=espacio_sup, espacio_inferior_pct=espacio_inf,
        hay_espacio_call=espacio_sup>=0.5, hay_espacio_put=espacio_inf>=0.5,
        volatilidad_abierta=pct_1h>=55,
        ma20_1h=round(ma20,2), ma40_1h=round(ma40,2), ma100_1h=round(ma100,2), ma200_1h=round(ma200,2),
    )
...
def score_e4_call(bb15, bb1h, bbdiario, chop):
    ...
    if bb15.mid_alcista: score+=20; criterios["15m_mid_alcista"]=True; notas.append("OK 15min: MA20 inclinada al alza")
    else: criterios["15m_mid_alcista"]=False; notas.append("WARN 15min: MA20 NO inclinada al alza")
    if bb15.bandas_abiertas: score+=15; criterios["15m_bandas_abiertas"]=True; notas.append(f"OK 15min: Bandas abiertas ({bb15.bandwidth_pct:.0f}%)")
    else: criterios["15m_bandas_abiertas"]=False; notas.append(f"WARN 15min: Bandas NO abiertas ({bb15.bandwidth_pct:.0f}%)")
    if bb15.precio_sobre_mid: score+=15; criterios["15m_precio_sobre_mid"]=True; notas.append(f"OK 15min: Precio ${bb15.precio} SOBRE MA20 ${bb15.mid}")
    else: criterios["15m_precio_sobre_mid"]=False; notas.append(f"WARN 15min: Precio BAJO MA20 -- NO CALL")
    if bb15.banda_inf_abriendo: score+=10; criterios["15m_fuerza_call"]=True; notas.append("OK 15min: Banda inferior abriendo -- fuerza alcista")
    else: criterios["15m_fuerza_call"]=False; notas.append("WARN 15min: Banda inferior no abriendo -- sin fuerza")
    ...
    if bb1h.tendencia in ["alcista_fuerte","alcista_parcial"]:
        score+=(15 if bb1h.tendencia=="alcista_fuerte" else 10); criterios["1h_tendencia_call"]=True; notas.append(f"OK 1H: Tendencia {bb1h.tendencia}")
    else: criterios["1h_tendencia_call"]=False; notas.append(f"WARN 1H: Tendencia {bb1h.tendencia}")
    if bb1h.hay_espacio_call: score+=8; criterios["1h_espacio_call"]=True; notas.append(f"OK 1H: Espacio superior {bb1h.espacio_superior_pct:.2f}%")
    else: criterios["1h_espacio_call"]=False; notas.append(f"WARN 1H: Poco espacio superior {bb1h.espacio_superior_pct:.2f}%")
    if bb1h.mid_alcista: score+=7; criterios["1h_mid_alcista"]=True; notas.append("OK 1H: MA20 inclinada al alza")
    else: criterios["1h_mid_alcista"]=False; notas.append("WARN 1H: MA20 no inclinada")
    criterios["1h_volatilidad"]=bb1h.volatilidad_abierta
    if bbdiario.tendencia in ["alcista_fuerte","alcista_parcial"]: score+=5; criterios["d_tendencia_call"]=True; notas.append(f"OK Diario: {bbdiario.tendencia}")
    else: criterios["d_tendencia_call"]=False
    if bbdiario.hay_espacio_call: score+=3; criterios["d_espacio_call"]=True
    else: criterios["d_espacio_call"]=False
    if bbdiario.precio_sobre_mid: score+=2; criterios["d_precio_sobre_mid"]=True
...
def score_e4_put(bb15, bb1h, bbdiario, chop):
    score=0.0; criterios={}; notas=[]
    if bb15.mid_bajista: score+=20; criterios["15m_mid_bajista"]=True; notas.append("OK 15min: MA20 inclinada a la baja")
    else: criterios["15m_mid_bajista"]=False; notas.append("WARN 15min: MA20 NO inclinada a la baja")
    if bb15.bandas_abiertas: score+=15; criterios["15m_bandas_abiertas"]=True; notas.append(f"OK 15min: Bandas abiertas ({bb15.bandwidth_pct:.0f}%)")
    else: criterios["15m_bandas_abiertas"]=False; notas.append(f"WARN 15min: Bandas NO abiertas ({bb15.bandwidth_pct:.0f}%)")
    if bb15.precio_bajo_mid: score+=15; criterios["15m_precio_bajo_mid"]=True; notas.append(f"OK 15min: Precio ${bb15.precio} BAJO MA20 ${bb15.mid}")
    else: criterios["15m_precio_bajo_mid"]=False; notas.append("WARN 15min: Precio SOBRE MA20 -- NO PUT")
    if bb15.banda_sup_abriendo: score+=10; criterios["15m_fuerza_put"]=True; notas.append("OK 15min: Banda superior abriendo -- fuerza bajista")
    else: criterios["15m_fuerza_put"]=False; notas.append("WARN 15min: Banda superior no abriendo")
    ...
    if bb1h.tendencia in ["bajista_fuerte","bajista_parcial"]:
        score+=(15 if bb1h.tendencia=="bajista_fuerte" else 10); criterios["1h_tendencia_put"]=True; notas.append(f"OK 1H: Tendencia {bb1h.tendencia}")
    else: criterios["1h_tendencia_put"]=False; notas.append(f"WARN 1H: Tendencia {bb1h.tendencia}")
    if bb1h.hay_espacio_put: score+=8; criterios["1h_espacio_put"]=True; notas.append(f"OK 1H: Espacio inferior {bb1h.espacio_inferior_pct:.2f}%")
    else: criterios["1h_espacio_put"]=False; notas.append(f"WARN 1H: Poco espacio inferior {bb1h.espacio_inferior_pct:.2f}%")
    if bb1h.mid_bajista: score+=7; criterios["1h_mid_bajista"]=True; notas.append("OK 1H: MA20 inclinada a la baja")
    else: criterios["1h_mid_bajista"]=False; notas.append("WARN 1H: MA20 no inclinada")
    criterios["1h_volatilidad"]=bb1h.volatilidad_abierta
    if bbdiario.tendencia in ["bajista_fuerte","bajista_parcial"]: score+=5; criterios["d_tendencia_put"]=True
    else: criterios["d_tendencia_put"]=False
    if bbdiario.hay_espacio_put: score+=3; criterios["d_espacio_put"]=True
    else: criterios["d_espacio_put"]=False
    if bbdiario.precio_bajo_mid: score+=2; criterios["d_precio_bajo_mid"]=True
...
def identify_e4(bb15, bb1h, bbdiario, chop):
    ...
    call_15m_ok = (crit_call.get("15m_mid_alcista",False) and
                   crit_call.get("15m_bandas_abiertas",False) and
                   crit_call.get("15m_precio_sobre_mid",False))
    call_1h_ok = sum([crit_call.get("1h_tendencia_call",False),
                      crit_call.get("1h_espacio_call",False),
                      crit_call.get("1h_mid_alcista",False)]) >= 2
    call_valido = call_15m_ok and call_1h_ok and score_call >= 60
    put_15m_ok = (crit_put.get("15m_mid_bajista",False) and
                  crit_put.get("15m_bandas_abiertas",False) and
                  crit_put.get("15m_precio_bajo_mid",False))
    put_1h_ok = sum([crit_put.get("1h_tendencia_put",False),
                     crit_put.get("1h_espacio_put",False),
                     crit_put.get("1h_mid_bajista",False)]) >= 2
    put_valido = put_15m_ok and put_1h_ok and score_put >= 60
    if call_valido and put_valido:
        if score_call >= score_put: put_valido = False
        else: call_valido = False
    if call_valido:
        strength = SignalStrength.FUERTE if score_call >= 80 else SignalStrength.MODERADO
        return (StrategyType.E4_CALL, SignalDirection.CALL, strength, score_call, crit_call, {}, exp_call)
    if put_valido:
        strength = SignalStrength.FUERTE if score_put >= 80 else SignalStrength.MODERADO
        return (StrategyType.E4_PUT, SignalDirection.PUT, strength, score_put, {}, crit_put, exp_put)
    return (StrategyType.NONE, SignalDirection.NEUTRAL, SignalStrength.DEBIL, max(score_call,score_put), crit_call, crit_put, "")
    ...
    try:
        ...
        if len(bw) >= 4:
            ...
            if all(last4[i] > last4[i+1] for i in range(3)):
                signals.append("BB 15min contrayendose -- volatilidad cediendo")
        if bb15.mid > 0:
            dist = abs(bb15.precio - bb15.mid) / bb15.mid * 100
            if dist > 3.0: signals.append(f"Precio {dist:.1f}% del punto medio -- extendido")
        if len(df_15m) >= 3:
            doji_count = 0
...
def generate_recommendation(direction, strength, bb15, bb1h, bbdiario, events, earnings, agotamiento, score, chop):
    if chop > 61.8: return "NO OPERAR -- Mercado choppy. Esperar."
    if earnings.get("has_earnings") and earnings.get("days_away") == 0:
        return f"NO ENTRAR -- EARNINGS HOY. IV Crush garantizado."
    ...
    base = (f"{direction.value} {'FUERTE' if strength==SignalStrength.FUERTE else 'MODERADO'} -- "
            f"Criterios del documento alineados.\n"
            f"Vol 15min: {bb15.bandwidth_pct:.0f}% | Espacio: {espacio:.2f}%\n"
...
def analyze_ticker(ticker):
    try:
        ...
        print(f"[{ticker}] Score:{score} | Vol:{bb15.volatilidad}({bb15.bandwidth_pct:.0f}%) | "
              f"Mid15:{mid_dir} | P/Mid:{p_mid} | Chop:{chop} | 1H:{bb1h.tendencia[:8]} | -> {strategy.name}")
        ...
        rec = generate_recommendation(direction, strength, bb15, bb1h, bbdiario, events, earnings, agotamiento, score, chop)
...
def run_analysis(tickers=None):
    ...
    et = pytz.timezone('US/Eastern')
    print(f"\n{'='*65}")
    print(f"  SAAI v6.0 -- Smart Alert AI System")
    print(f"  E4 -- Saliendo de Bollinger Bands")
    print(f"  Basado en documento de metodologia personal")
